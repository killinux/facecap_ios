...
4. **上门牙被上唇挡住**：上前牙长在上颌（颅骨），转下颌骨不动它；而上唇若完全锁死，
   其下边缘会垂下挡住上门牙，正面看「露不出门牙」。需让上唇中央随张嘴**略上抬**
   （自然张嘴时上唇本就会抬）。
...
5. **补漏（只填洞）**：在「唇缝以下、贴嘴前部」的小盒区内，**只填补漏网静止点**——自身
   位移≈0 却被运动顶点包围的洞，从邻居向内扩散填充。**绝不改动本来运动正确的顶点**。
   ⚠️ 教训：早期版本对整片区域做拉普拉斯平滑，会把嘴角（上下唇交汇处）本来干净的运动
   顶点也搅乱、拉出尖刺撕裂感（纯骨骼旋转的嘴角本是干净的）。改成「只填洞」后两全其美。
6. **归一化**：把满 weight 最大张嘴位移缩到 `target_mm`(默认 14mm，对齐 Children 原生)。
7. **上唇抬升**：上唇中央（门牙宽度）按 `upper_lip_lift_mm`(默认 5mm) 微抬，露出上门牙。
   按「高度」平滑抬升（唇线以上才抬、峰在唇缘、向上渐隐）→ 边缘平顺。
   ⚠️ 教训：早期用「运动门控」决定抬哪些点，会在抬/不抬边界产生锯齿波浪（上唇中间不平）；
   唇线测准后改用纯高度场即可。

**真实唇线定位（关键）**：上面所有「唇缝/上唇」操作都依赖唇线高度。**别写死偏移**——各模型
嘴的位置不同（实测 AC 真唇线比 Children 标定高 6mm）。`detect_lip_line` 从下颌运动边界
动态测：取中央前脸「下颌运动顶点的最高处」，再加缺口余量（下唇内缘权重缺口让运动最高点
落在真唇线下方）。日志 `唇线y=...mm(相对眼)` 可核对。
...
- `upper_lip_lift_mm`：上唇抬升量（露门牙）。门牙仍被挡就加大，上唇翘太多就减小；0 关闭。
